...
if [[ "$DEVICE" == true && "$AOT" == true ]]; then
    echo "=== Building BlinkIDUX tests for device (NativeAOT) ==="
    ...
    echo "=== Building BlinkIDUX tests for device ==="
    dotnet build . -c Debug -p:RuntimeIdentifier=ios-arm64
else
    echo "=== Building BlinkIDUX tests for simulator ==="
