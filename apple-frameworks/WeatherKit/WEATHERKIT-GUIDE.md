...
- **Historical statistics & summaries.** `WeatherService.dailyStatistics`, `monthlyStatistics`, `hourlyStatistics`, and `dailySummary` are not emitted. The associated types (`DailyWeatherStatistics<T>`, `MonthlyWeatherStatistics<T>`, `HourlyWeatherStatistics<T>`, `DailyWeatherSummary<T>`, `DayTemperatureStatistics`, `DayPrecipitationStatistics`, etc.) are present in the namespace but have no service method to produce them.
