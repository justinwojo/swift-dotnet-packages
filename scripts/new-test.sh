#!/bin/bash
# Scaffold a new test app from the library-test template.
# Tests are co-located under libraries/<LibraryName>/tests/.
#
# Usage: ./scripts/new-test.sh <LibraryName> [options]
#
# Arguments:
...
TEMPLATE_DIR="$REPO_ROOT/templates/library-test"
...
[ -n "$LIBRARY_NAME" ] || die "LibraryName is required. Usage: ./scripts/new-test.sh <LibraryName> [--products P1,P2] [--all-products] [--with <root>[:P1,P2]] [--module M] [--force]"
...
TARGET_DIR="$REPO_ROOT/libraries/${LIBRARY_NAME}/tests"
...
generate_csproj() {
    ...
    local csproj_path="$TARGET_DIR/SwiftBindings.${LIBRARY_NAME}.Tests.csproj"
...
CSPROJ_HEADER
    ...
    echo "    <ApplicationId>com.swiftbindings.${LIBRARY_NAME_LOWER}.tests</ApplicationId>" >> "$csproj_path"
...
CSPROJ_PROPS
    ...
    for entry in "${RESOLVED_PRODUCTS[@]}"; do
        ...
        # Cross-library deps (--with) need a different relative path
        if [ "$lib_name" != "$LIBRARY_NAME" ]; then
            rel_dir="../../${lib_name}"
            [ -n "$subdir" ] && rel_dir="$rel_dir/$subdir"
        fi
...
CSPROJ_REFS_END
    ...
    for entry in "${RESOLVED_PRODUCTS[@]}"; do
        ...
        if [ "$lib_name" != "$LIBRARY_NAME" ]; then
            base_path="../../${lib_name}"
            [ -n "$subdir" ] && base_path="$base_path/$subdir"
        else
            base_path=".."
            [ -n "$subdir" ] && base_path="$base_path/$subdir"
        fi
...
echo "Created libraries/${LIBRARY_NAME}/tests/"
echo ""
...
echo ""
echo "Next steps:"
echo "  1. Build library:     dotnet nuke BuildLibrary --library ${LIBRARY_NAME}"
echo "  2. Build test app:    dotnet nuke BuildTestApp --library ${LIBRARY_NAME}"
echo "     For device:        dotnet nuke BuildTestApp --library ${LIBRARY_NAME} --device"
echo "     (The SDK csproj generates bindings automatically during build)"
echo "  3. Validate (sim):    dotnet nuke ValidateSim --library ${LIBRARY_NAME} --timeout 15"
echo "     Validate (device): dotnet nuke ValidateDevice --library ${LIBRARY_NAME} --timeout 30"
